...
if (staffProfile && req.method === 'PATCH') {
  ...
  if (orderPath && req.method === 'POST' && orderPath[2] === 'discount-requests') {
    ...
    const request = { id: `disc-${Date.now()}`, orderId: order.id, type, value, reason, guestName: order.guestName || null, guestPhone: order.guestPhone || null, status: 'approved', requestedBy: input.requestedBy || req.user?.name || 'unknown', approvedBy: req.user?.name || 'сотрудник', createdAt: new Date().toISOString(), decidedAt: new Date().toISOString(), notificationRecipients: ['owner', 'admin'] };
