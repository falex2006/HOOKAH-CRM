...
async function api(req, res) {
  ...
  if (pathname === '/api/clients' && req.method === 'GET') {
    ...
    if (repositories?.pool) { try { const { rows } = await repositories.pool.query(`SELECT g.id,g.full_name AS name,g.phone,g.email,g.avatar_url AS "avatarUrl",g.phone_numbers AS "phoneNumbers",g.telegram,g.tobacco_preferences AS "tobaccoPreferences",g.bowl_preferences AS "bowlPreferences",g.bar_preferences AS "barPreferences",g.allergies,g.loyalty_points AS "loyaltyPoints",g.notes,COUNT(DISTINCT o.id)::int AS visits,COALESCE(SUM(p.amount),0)::numeric AS "totalSpent",MAX(o.closed_at) AS "lastVisitAt" FROM guests g LEFT JOIN orders o ON o.guest_id=g.id AND o.status='closed' LEFT JOIN payments p ON p.order_id=o.id AND p.status IN ('paid','partially_paid') WHERE g.venue_id=$1 AND ($2='' OR LOWER(CONCAT_WS(' ',g.full_name,g.phone,g.telegram,g.phone_numbers::text,ARRAY_TO_STRING(COALESCE(g.tobacco_preferences,'{}'),' '),ARRAY_TO_STRING(COALESCE(g.bowl_preferences,'{}'),' '),ARRAY_TO_STRING(COALESCE(g.bar_preferences,'{}'),' '),g.allergies,g.notes)) LIKE '%'||LOWER($2)||'%') GROUP BY g.id ORDER BY COALESCE(MAX(o.closed_at),g.created_at) DESC`, [venueDbId, query]); return json(res, 200, { items: rows.map((row) => ({ ...row, phoneNumbers: row.phoneNumbers || (row.phone ? [{ label: 'Основной', number: row.phone, primary: true }] : []), tobaccoPreferences: row.tobaccoPreferences || [], bowlPreferences: row.bowlPreferences || [], barPreferences: row.barPreferences || [], loyaltyPoints: Number(row.loyaltyPoints || 0), visits: Number(row.visits || 0), totalSpent: Number(row.totalSpent || 0) })) }); } catch (_) {} }
  ...
  if (pathname === '/api/clients' && req.method === 'POST') {
    ...
    const client = { id: `client-${Date.now()}`, name, avatarUrl, phoneNumbers, telegram: String(input.telegram || '').trim(), tobaccoPreferences: Array.isArray(input.tobaccoPreferences) ? input.tobaccoPreferences.map(String).map((item) => item.trim()).filter(Boolean).slice(0, 30) : [], bowlPreferences: Array.isArray(input.bowlPreferences) ? input.bowlPreferences.map(String).map((item) => item.trim()).filter(Boolean).slice(0, 20) : [], barPreferences: Array.isArray(input.barPreferences) ? input.barPreferences.map(String).map((item) => item.trim()).filter(Boolean).slice(0, 30) : [], allergies: String(input.allergies || '').trim().slice(0, 500), notes: String(input.notes || '').trim().slice(0, 2000), loyaltyPoints: 0, visits: 0, totalSpent: 0, lastVisitAt: null };
    if (repositories?.pool) { try { const primary = phoneNumbers.find((phone) => phone.primary)?.number || null; const { rows } = await repositories.pool.query(`INSERT INTO guests (venue_id,phone,full_name,avatar_url,phone_numbers,telegram,tobacco_preferences,bowl_preferences,bar_preferences,allergies,notes) VALUES ($1,$2,$3,$4,$5::jsonb,$6,$7,$8,$9,$10,$11) RETURNING id,full_name AS name,phone,avatar_url AS "avatarUrl",phone_numbers AS "phoneNumbers",telegram,tobacco_preferences AS "tobaccoPreferences",bowl_preferences AS "bowlPreferences",bar_preferences AS "barPreferences",allergies,notes,loyalty_points AS "loyaltyPoints"`, [venueDbId, primary, name, client.avatarUrl || null, JSON.stringify(phoneNumbers), client.telegram || null, client.tobaccoPreferences, client.bowlPreferences, client.barPreferences, client.allergies || null, client.notes || null]); if (rows[0]) return json(res, 201, { ...client, ...rows[0] }); } catch (_) {} }
  ...
  if (clientProfile && req.method === 'PATCH') {
    ...
    if (!client && repositories?.pool && /^[0-9a-f-]{36}$/i.test(clientProfile[1])) {
      try {
        const { rows } = await repositories.pool.query(`SELECT id,full_name AS name,phone,avatar_url AS "avatarUrl",phone_numbers AS "phoneNumbers",telegram,tobacco_preferences AS "tobaccoPreferences",bowl_preferences AS "bowlPreferences",bar_preferences AS "barPreferences",allergies,notes,loyalty_points AS "loyaltyPoints" FROM guests WHERE id=$1 AND venue_id=$2`, [clientProfile[1], venueDbId]);
    ...
    if (repositories?.pool && /^[0-9a-f-]{36}$/i.test(client.id)) { try { const primary = client.phoneNumbers.find((phone) => phone.primary)?.number || null; const { rows } = await repositories.pool.query(`UPDATE guests SET phone=$1,full_name=$2,avatar_url=$3,phone_numbers=$4::jsonb,telegram=$5,tobacco_preferences=$6,bowl_preferences=$7,bar_preferences=$8,allergies=$9,notes=$10 WHERE id=$11 AND venue_id=$12 RETURNING id,full_name AS name,phone,avatar_url AS "avatarUrl",phone_numbers AS "phoneNumbers",telegram,tobacco_preferences AS "tobaccoPreferences",bowl_preferences AS "bowlPreferences",bar_preferences AS "barPreferences",allergies,notes,loyalty_points AS "loyaltyPoints"`, [primary, client.name, client.avatarUrl || null, JSON.stringify(client.phoneNumbers), client.telegram || null, client.tobaccoPreferences, client.bowlPreferences, client.barPreferences, client.allergies || null, client.notes || null, client.id, venueDbId]); if (rows[0]) return json(res, 200, { ...client, ...rows[0] }); } catch (_) {} }
