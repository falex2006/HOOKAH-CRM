...
async function api(req, res) {
  ...
  if (pathname === '/api/analytics' && req.method === 'GET') {
    ...
    if (repositories?.pool) {
      try {
        const [daily, products, hall, staffRows, stationRows, expenseRows] = await Promise.all([
          ...
          repositories.pool.query(`SELECT expense_date AS date, COALESCE(SUM(amount),0) AS amount FROM expenses WHERE venue_id=$1 AND expense_date BETWEEN $2::date AND $3::date GROUP BY expense_date`, [venueDbId, startDate, endDate])
        ]);
        const expensesByDate = Object.fromEntries(expenseRows.rows.map((row) => [String(row.date).slice(0, 10), Number(row.amount || 0)])); const days = daily.rows.map((row) => { const date = String(row.date).slice(0, 10); const revenue = Number(row.revenue || 0); const expenses = expensesByDate[date] || 0; const orders = Number(row.orders || 0); return { date, revenue, expenses, netProfit: revenue - expenses, orders, averageCheck: orders ? revenue / orders : 0, medianCheck: orders ? revenue / orders : 0, tables: 0 }; }); const totalRevenue = days.reduce((sum, row) => sum + row.revenue, 0); const totalExpenses = days.reduce((sum, row) => sum + row.expenses, 0); const totalOrders = days.reduce((sum, row) => sum + row.orders, 0); const hallRow = hall.rows[0] || {};
