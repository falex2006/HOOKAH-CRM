...
| Визуальные регрессии CSS, action-ссылок и иконок | Подтверждено локально | `local-design-contract.mjs` — актуальный `style.css?rev=130`, без подчёркиваний, с focus-state и встроенными Tabler Icons |
...
- Числовые поля и системные спиннеры | Подтверждено локально | CSS rev=130, единый тёмный стиль для staff/Velora
