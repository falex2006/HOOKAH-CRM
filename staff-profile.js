(function mountStaffProfile(){
  ...
  const readUser=()=>{try{return JSON.parse(localStorage.getItem(storageKey)||'{}');}catch(_){return{};}};
  const writeUser=(user)=>{localStorage.setItem(storageKey,JSON.stringify(user));};
  const sourceOf=(user)=>user.avatarUrl||user.avatar_url||user.avatar||user.photoUrl||user.photo_url||user.photo||'';
  const initials=(user)=>String(user.name||user.fullName||user.username||'С').trim().slice(0,1).toUpperCase();
  const renderAvatar=(host,user)=>{
    if(!host)return;
    let avatar=host.querySelector('.staff-profile-avatar');
    if(!avatar){avatar=document.createElement('span');avatar.className='staff-profile-avatar';host.prepend(avatar);}
    const source=sourceOf(user);
    avatar.textContent=source?'':initials(user);
    avatar.style.backgroundImage=source?'url("'+source.replace(/"/g,'')+'")':'';
  };
  const open=()=>{
    ...
    modal.innerHTML='<div class="staff-profile-dialog" role="dialog" aria-modal="true" aria-label="Профиль сотрудника"><button type="button" class="staff-profile-close" aria-label="Закрыть">×</button><div class="staff-profile-dialog__heading"><span class="staff-profile-dialog__avatar staff-profile-preview">'+initials(user)+'</span><div><h2>Мой профиль</h2><p>Аватар и рабочие контакты</p></div></div><label class="staff-profile-field">Аватар<input type="file" class="staff-profile-file" accept="image/png,image/jpeg,image/webp"></label><label class="staff-profile-field">Telegram<input type="text" class="staff-profile-telegram" placeholder="@username или https://t.me/username"></label><p class="staff-profile-error" role="alert"></p><div class="staff-profile-actions"><button type="button" class="staff-profile-cancel">Отмена</button><button type="button" class="staff-profile-save">Сохранить</button></div></div>';
    document.body.append(modal);
    const preview=modal.querySelector('.staff-profile-preview'); const file=modal.querySelector('.staff-profile-file'); const telegram=modal.querySelector('.staff-profile-telegram'); const error=modal.querySelector('.staff-profile-error');
