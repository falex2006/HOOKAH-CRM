...
async function api(req, res) {
  ...
  if (pathname === '/api/floor/tables' && req.method === 'POST') {
    ...
    if (repositories?.pool && /^[0-9a-f-]{36}$/i.test(zoneId)) { try { const { rows } = await repositories.pool.query('INSERT INTO tables (zone_id,name,capacity,min_deposit,min_order_total) SELECT id,$2,$3,$4,$4 FROM zones WHERE id=$1 AND venue_id=$5 RETURNING id,name,status,capacity,min_order_total,layout', [zoneId, name, capacity, minimumOrderTotal, venueDbId]); if (!rows[0]) return json(res, 404, { error: 'zone_not_found' }); await repositories.pool.query(`WITH ordered AS (SELECT t.id,row_number() OVER (ORDER BY NULLIF(regexp_replace(t.name, '\\D', '', 'g'), '')::int NULLS LAST, t.id) AS number FROM tables t JOIN zones z ON z.id=t.zone_id WHERE t.zone_id=$1 AND z.venue_id=$2 AND z.name NOT ILIKE '%vip%' AND z.name NOT ILIKE '%комнат%') UPDATE tables t SET name='Стол ' || ordered.number FROM ordered WHERE t.id=ordered.id`, [zoneId, venueDbId]); const { rows: updatedRows } = await repositories.pool.query('SELECT id,name,status,capacity,min_order_total,layout FROM tables WHERE id=$1', [rows[0].id]); const table = { ...updatedRows[0], minimumOrderTotal: Number(updatedRows[0].min_order_total), layout: updatedRows[0].layout || {} }; recordAudit(req, 'floor_table.created', 'table', table.id, null, table); return json(res, 201, table); } catch (error) { return json(res, 409, { error: 'table_create_failed', detail: error.message }); } }
  ...
  if (pathname === '/api/floor') {
    ...
    if (repositories?.pool) { try { const { rows } = await repositories.pool.query(`SELECT z.id AS zone_id,z.name AS zone_name,z.sort_order,t.id,t.name,CASE WHEN t.status='blocked' THEN 'blocked' WHEN EXISTS (SELECT 1 FROM orders o WHERE o.table_id=t.id AND o.venue_id=$1 AND o.status IN ('open','in_progress','ready')) THEN 'occupied' ELSE t.status END AS status,t.capacity,t.min_order_total,t.layout FROM zones z JOIN tables t ON t.zone_id=z.id WHERE z.venue_id=$1 ORDER BY z.sort_order,NULLIF(regexp_replace(t.name, '\\D', '', 'g'), '')::int NULLS LAST,t.name`, [venueDbId]); const zones = []; for (const row of rows) { let zone = zones.find((entry) => entry.id === row.zone_id); if (!zone) { zone = { id: row.zone_id, name: row.zone_name, tables: [] }; zones.push(zone); } zone.tables.push({ id: row.id, name: row.name, status: row.status, capacity: row.capacity, minimumOrderTotal: Number(row.min_order_total), layout: row.layout || {} }); } return json(res, 200, { zones }); } catch (_) {} }
