...
- `COMPLETION_MATRIX.md` — матрица требований, доказательств и незавершённых внешних этапов;
...
## SaaS Control Center
Платформенный владелец входит через SAAS_OWNER_EMAIL/SAAS_OWNER_PASSWORD и попадает на /platform. Секреты задаются в .env или менеджере секретов VPS; реальные значения не хранятся в Git. Control Center показывает организации, тарифы, состояние и создаёт новые tenant-контуры через /api/platform/organizations.
