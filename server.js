...
async function api(req, res) {
  ...
  if (pathname === '/api/venue' && (req.method === 'PATCH' || req.method === 'PUT')) {
    ...
    if (repositories?.pool) { try { await repositories.pool.query('UPDATE venues SET name=$1,city=$2,format=$3,phone=$4,address=$5,timezone=$6,logo_url=$7 WHERE id=$8', [venue.name, venue.city, venue.format, venue.phone, venue.address, venue.timezone, venue.logoUrl, venueDbId]); } catch (error) { Object.assign(venue, before); return json(res, 503, { error: 'venue_save_failed', detail: error.message }); } }
