...
async function api(req, res) {
  ...
  if (pathname === '/api/recipes' && req.method === 'GET') {
    if (denyUnless(req, res, 'inventory_read')) return;
    if (repositories?.pool) { try { const { rows } = await repositories.pool.query('SELECT id,product_id AS "productId",name,ingredients,technology,serve,yield_quantity AS "yieldQuantity",yield_unit AS "yieldUnit",portion_count AS "portionCount",recipe_type AS "recipeType",active,created_at AS "createdAt",updated_at AS "updatedAt" FROM inventory_recipe_cards WHERE venue_id=$1 AND active=true ORDER BY name', [venueDbId]); return json(res, 200, { items: rows }); } catch (error) { return json(res, 503, { error: 'recipes_unavailable', detail: error.message }); } }
...
if (staffProfile && req.method === 'PATCH') {
  ...
  if (pathname === '/api/inventory/premixes' && req.method === 'GET') {
    ...
    if (repositories?.pool) { try { const { rows } = await repositories.pool.query(`SELECT b.id,b.recipe_id AS "recipeId",r.name AS "recipeName",b.output_ingredient_id AS "outputItemId",i.name AS "outputItemName",b.output_quantity AS "outputQuantity",b.output_unit AS "outputUnit",b.total_cost AS "totalCost",b.ingredients,b.created_at AS "createdAt" FROM inventory_premix_batches b JOIN inventory_recipe_cards r ON r.id=b.recipe_id JOIN ingredients i ON i.id=b.output_ingredient_id WHERE b.venue_id=$1 ORDER BY b.created_at DESC LIMIT 50`, [venueDbId]); return json(res, 200, { items: rows }); } catch (error) { return json(res, 503, { error: 'premixes_unavailable', detail: error.message }); } }
    return json(res, 200, { items: stockMovements.filter((item) => item.type === 'premix').slice(-50).reverse() });
  }
  if (pathname === '/api/inventory/premixes/produce' && req.method === 'POST') {
    ...
    if (!recipeId || !outputItemId || !Number.isFinite(multiplier) || multiplier <= 0 || multiplier > 1000) return json(res, 400, { error: 'invalid_premix_production' });
    const unitFactors = { г: { г: 1, кг: 0.001 }, кг: { кг: 1, г: 1000 }, мл: { мл: 1, л: 0.001 }, л: { л: 1, мл: 1000 }, шт: { шт: 1 }, порция: { порция: 1 }, уп: { уп: 1 }, упаковка: { упаковка: 1 } };
    if (repositories?.pool) {
      let client;
      try {
        ...
        const outputFactor = unitFactors[recipe.yieldUnit]?.[output.unit]; if (!outputFactor) return json(res, 409, { error: 'premix_output_unit_mismatch', sourceUnit: recipe.yieldUnit, targetUnit: output.unit }); const outputQuantity = Number((Number(recipe.yieldQuantity) * multiplier * outputFactor).toFixed(6));
        ...
        for (const requirement of requirements) {
          ...
          requirement.item = row; totalCost += requirement.quantity * Number(row.cost || 0);
        }
        ...
        for (const requirement of requirements) await client.query('INSERT INTO stock_movements (venue_id,ingredient_id,direction,quantity,reason,created_by) VALUES ($1,$2,\'out\',$3,$4,$5)', [venueDbId, requirement.item.id, requirement.quantity, `Приготовление премикса «${recipe.name}»`, /^[0-9a-f-]{36}$/i.test(req.user?.id || '') ? req.user.id : null]);
        await client.query('INSERT INTO stock_movements (venue_id,ingredient_id,direction,quantity,reason,created_by) VALUES ($1,$2,\'in\',$3,$4,$5)', [venueDbId, output.id, outputQuantity, `Выход премикса «${recipe.name}»`, /^[0-9a-f-]{36}$/i.test(req.user?.id || '') ? req.user.id : null]);
        ...
        await client.query('UPDATE ingredients SET cost=$1 WHERE id=$2 AND venue_id=$3', [Math.round(nextOutputCost * 100) / 100, output.id, venueDbId]);
