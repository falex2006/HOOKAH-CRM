...
- Статус: коммит `2152de962547a914cccc22092c067fd79041d858` отправлен в `origin/main`. VPS отвечает по HTTP (`/api/health` — 200), но не обновлён: SSH из текущей среды возвращает `Permission denied (publickey,password)`. Штатный deploy требует `COOKIE_SECURE=true`, совместимый с HTTPS; HTTPS и домен ранее отложены. Защитную настройку не ослаблял.
