...
if (staffProfile && req.method === 'PATCH') {
  ...
  if (pathname === '/api/reservations' && req.method === 'POST') {
    ...
    const reservation = { id: `res-${Date.now()}`, guestName: input.guestName, phone: input.phone || '', date: input.date, time: input.time, tableId: input.tableId, tableName, guests: Number(input.guests || 1), status: 'confirmed', deposit, notes: input.notes || '', createdBy: req.user?.id || null, createdByName: req.user?.name || 'Сотрудник', createdByRole: req.user?.role === 'owner' ? 'Владелец' : req.user?.role === 'admin' ? 'Администратор' : 'Сотрудник', createdAt: new Date().toISOString() };
