...
try {
 ...
 await req(`/api/inventory/auto-orders/${request.id}`,'PATCH',{status:'received'},409);
 await req(`/api/inventory/auto-orders/${request.id}`,'PATCH',{status:'partially_received',receipts:[{itemId:item.id,quantity:250}]},409);
 assert.equal((await req('/api/inventory')).items.find((entry) => entry.id === item.id).onHand,0); checks++;
