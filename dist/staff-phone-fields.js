(function mountStaffPhoneList(){
  const mount=()=>{
    ...
    inputs.forEach((input)=>{
      ...
      input.parentElement?.after(rows); input.style.display='none';
      const list=rows.querySelector('.staff-phone-list__rows'); const hidden=rows.querySelector('.staff-phone-list__value');
      ...
      const add=(item={label:'Дополнительный',number:'',primary:false})=>{const row=document.createElement('div'); row.className='staff-phone-row'; row.innerHTML='<select aria-label="Тип телефона"><option>Рабочий</option><option>Личный</option><option>Резервный</option><option>Дополнительный</option></select><input type="tel" placeholder="+7 (___) ___-__-__" aria-label="Номер телефона"><label><input type="radio" name="staff_primary_phone" aria-label="Основной"> основной</label><button type="button" class="staff-phone-row__remove" aria-label="Удалить номер">×</button>'; row.querySelector('select').value=item.label||'Дополнительный'; row.querySelector('input[type="tel"]').value=item.number||''; row.querySelector('input[type="radio"]').checked=!!item.primary; row.addEventListener('input',sync); row.addEventListener('change',sync); row.querySelector('.staff-phone-row__remove').addEventListener('click',()=>{if(list.children.length>1){row.remove();sync();}}); list.append(row);};
      add(initial); rows.querySelector('.staff-phone-list__add').addEventListener('click',()=>add()); form.addEventListener('submit',sync); sync();
    });
