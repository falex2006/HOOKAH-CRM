...
class ReservationRepository {
  ...
  async create(input) {
    ...
    try {
      await client.query('BEGIN');
      const guest = await client.query(`INSERT INTO guests (venue_id, phone, full_name) VALUES ($1,$2,$3) ON CONFLICT (phone) DO UPDATE SET full_name=EXCLUDED.full_name, venue_id=EXCLUDED.venue_id RETURNING id`, [input.venueId, input.phone || null, input.guestName]);
