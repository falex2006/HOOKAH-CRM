...
| Визуальные регрессии CSS, action-ссылок и иконок | Подтверждено локально | `local-design-contract.mjs` — актуальный `style.css?rev=181`, без подчёркиваний, с focus-state и встроенными Tabler Icons |
...
| Блокировка рабочего места и PIN-разблокировка | Подтверждено локально | Ледяной стеклянный экран, персональный интервал 1/5/10/15/30 минут или отключение, ручная блокировка, клавиатура/тач-панель, отказ неверного и незаданного PIN; `local-lock-contract.mjs` и полный acceptance |`r`n| Регрессии на GitHub | Подтверждено | `.github/workflows/crm-contracts.yml`; run `35651251737` для `f83adbb` завершён успешно, включая локальный API для role/insights/date-контрактов |
...
- Кэширование административной логики | Подтверждено локально | Все portal HTML используют portal.js rev=172, stale-cache проверяется контрактом

- Кэш рабочего места | Подтверждено локально | index.html app.js rev=101 проверяется design contract

| Мобильная боковая навигация | Подтверждено локально | На узкой ширине иконки сохраняют title/aria-label с названием раздела; проверено статическим контрактом и JS-синтаксисом (portal.js rev=172) |
...
| Мобильная карточка выручки | Исправлено и проверено браузером | На viewport 390×844 блок ожидания не перекрывает сумму; мобильная раскладка вертикальная, CSS rev=179 |

| Планшетная шапка | Исправлено и проверено браузером | При 768×1024 второстепенные поля скрываются без обрезанного имени; основные действия остаются видимыми (CSS rev=180) |
