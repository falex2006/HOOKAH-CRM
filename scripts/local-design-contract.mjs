...
for (const file of htmlFiles) {
  ...
  assert.match(html, /style\.css\?rev=177/, `${file} must use current CSS cache version`);
  assert.doesNotMatch(html, /style\.css\?rev=(?:12[0-7]|1[01]\d)/, `${file} has stale CSS cache version`);
  if (file !== 'index.html' && file !== 'login.html' && file !== 'platform.html') assert.match(html, /portal\.js\?rev=166/, `${file} must use current portal JS cache version`);
...
console.log(`LOCAL DESIGN CONTRACT: PASS (routes=${htmlFiles.length}, CSS rev=177, action links and Tabler Icons)`);
