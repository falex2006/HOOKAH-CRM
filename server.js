...
async function api(req, res) {
  ...
  if (pathname === '/api/metrics') {
    if (repositories?.pool) {
      try {
        ...
        // Keep the empty workspace usable when an optional metric query is not
        // available in a fresh schema. The in-memory zero state below is the
        // correct representation until the first business records are added.
      }
    }
  ...
  if (pathname === '/api/analytics' && req.method === 'GET') {
    ...
    const requestedDays = Number(url.searchParams.get('days') || 7); const analyticsDays = Math.min(Math.max(Number.isFinite(requestedDays) ? Math.round(requestedDays) : 7, 3), 90); const end = new Date(); const start = new Date(end); start.setDate(start.getDate() - (analyticsDays - 1)); const startDate = start.toISOString().slice(0, 10); const endDate = end.toISOString().slice(0, 10);
    if (repositories?.pool) {
      try {
        const [daily, products, hall, staffRows, stationRows, expenseRows, costRows] = await Promise.all([
  ...
  if (floorTablePath && req.method === 'DELETE') {
    ...
    if (repositories?.pool && /^[0-9a-f-]{36}$/i.test(tableId)) { try { const { rows: active } = await repositories.pool.query("SELECT 1 FROM orders WHERE table_id=$1 AND venue_id=$2 AND status IN ('open','in_progress','ready') LIMIT 1", [tableId, venueDbId]); if (active[0]) return json(res, 409, { error: 'table_in_use' }); const { rows } = await repositories.pool.query('DELETE FROM tables WHERE id=$1 AND zone_id IN (SELECT id FROM zones WHERE venue_id=$2) RETURNING id,name,zone_id', [tableId, venueDbId]); if (!rows[0]) return json(res, 404, { error: 'table_not_found' }); await repositories.pool.query(`WITH ordered AS (SELECT t.id,row_number() OVER (ORDER BY NULLIF(regexp_replace(t.name, '\\D', '', 'g'), '')::int NULLS LAST, t.id) AS number FROM tables t JOIN zones z ON z.id=t.zone_id WHERE t.zone_id=$1 AND z.venue_id=$2 AND z.name NOT ILIKE '%vip%' AND z.name NOT ILIKE '%комнат%') UPDATE tables t SET name='Стол ' || ordered.number FROM ordered WHERE t.id=ordered.id`, [rows[0].zone_id, venueDbId]); recordAudit(req, 'floor_table.deleted', 'table', tableId, rows[0], null); return json(res, 200, rows[0]); } catch (error) { return json(res, 409, { error: 'table_delete_failed', detail: error.message }); } }
    const zone = floor.find((entry) => entry.tables.some((entry) => entry.id === tableId)); const table = zone?.tables.find((entry) => entry.id === tableId); if (!table) return json(res, 404, { error: 'table_not_found' }); if (orders.some((order) => order.tableId === tableId && ['open', 'in_progress', 'ready'].includes(order.status))) return json(res, 409, { error: 'table_in_use' }); zone.tables.splice(zone.tables.indexOf(table), 1); if (!/vip|комнат/i.test(zone.name)) zone.tables.forEach((entry, index) => { entry.name = `Стол ${index + 1}`; }); recordAudit(req, 'floor_table.deleted', 'table', tableId, table, null); return json(res, 200, table);
  }
  if (pathname === '/api/floor') {
    if (denyUnless(req, res, 'floor')) return;
    if (repositories?.pool) { try { const { rows } = await repositories.pool.query(`SELECT z.id AS zone_id,z.name AS zone_name,z.sort_order,t.id,t.name,CASE WHEN t.status='blocked' THEN 'blocked' WHEN EXISTS (SELECT 1 FROM orders o WHERE o.table_id=t.id AND o.venue_id=$1 AND o.status IN ('open','in_progress','ready')) THEN 'occupied' ELSE t.status END AS status,t.capacity,t.min_capacity,t.max_capacity,t.min_order_total,t.layout FROM zones z JOIN tables t ON t.zone_id=z.id WHERE z.venue_id=$1 ORDER BY z.sort_order,NULLIF(regexp_replace(t.name, '\\D', '', 'g'), '')::int NULLS LAST,t.name`, [venueDbId]); const zones = []; for (const row of rows) { let zone = zones.find((entry) => entry.id === row.zone_id); if (!zone) { zone = { id: row.zone_id, name: row.zone_name, tables: [] }; zones.push(zone); } zone.tables.push({ id: row.id, name: row.name, status: row.status, capacity: row.capacity, minCapacity: Number(row.min_capacity || row.capacity), maxCapacity: Number(row.max_capacity || row.capacity), minimumOrderTotal: Number(row.min_order_total), layout: row.layout || {} }); } return json(res, 200, { zones }); } catch (_) {} }
  ...
  if (pathname === '/api/recipes' && req.method === 'GET') {
    if (denyUnless(req, res, 'inventory_read')) return;
    if (repositories?.pool) { try { const { rows } = await repositories.pool.query('SELECT id,product_id AS "productId",name,ingredients,technology,serve,active,created_at AS "createdAt",updated_at AS "updatedAt" FROM inventory_recipe_cards WHERE venue_id=$1 AND active=true ORDER BY name', [venueDbId]); return json(res, 200, { items: rows }); } catch (error) { return json(res, 503, { error: 'recipes_unavailable', detail: error.message }); } }
  ...
  if (pathname === '/api/recipes' && req.method === 'POST') {
    ...
    if (repositories?.pool) { try { const productId = /^[0-9a-f-]{36}$/i.test(String(input.productId || '')) ? String(input.productId) : null; const { rows } = await repositories.pool.query('INSERT INTO inventory_recipe_cards (venue_id,product_id,name,ingredients,technology,serve) VALUES ($1,$2,$3,$4::jsonb,$5,$6) RETURNING id,product_id AS "productId",name,ingredients,technology,serve,active,created_at AS "createdAt",updated_at AS "updatedAt"', [venueDbId, productId, name, JSON.stringify(ingredients), technology || null, serve || null]); recordAudit(req, 'recipe.created', 'recipe', rows[0].id, null, rows[0]); return json(res, 201, rows[0]); } catch (error) { return json(res, 409, { error: 'recipe_save_failed', detail: error.message }); } }
  ...
  if (pathname === '/api/clients' && req.method === 'GET') {
    ...
    if (repositories?.pool) { try { const { rows } = await repositories.pool.query(`SELECT g.id,g.full_name AS name,g.phone,g.email,g.avatar_url AS "avatarUrl",g.guest_status AS "guestStatus",g.archived_at AS "archivedAt",g.phone_numbers AS "phoneNumbers",g.telegram,g.tobacco_preferences AS "tobaccoPreferences",g.bowl_preferences AS "bowlPreferences",g.bar_preferences AS "barPreferences",g.allergies,g.loyalty_points AS "loyaltyPoints",g.notes,COUNT(DISTINCT o.id)::int AS visits,COALESCE(SUM(p.amount),0)::numeric AS "totalSpent",MAX(o.closed_at) AS "lastVisitAt" FROM guests g LEFT JOIN orders o ON o.guest_id=g.id AND o.status='closed' AND o.closed_at >= $3::timestamptz LEFT JOIN payments p ON p.order_id=o.id AND p.status IN ('paid','partially_paid') WHERE g.venue_id=$1 AND g.archived_at IS NULL AND ($2='' OR LOWER(CONCAT_WS(' ',g.full_name,g.phone,g.telegram,g.phone_numbers::text,ARRAY_TO_STRING(COALESCE(g.tobacco_preferences,'{}'),' '),ARRAY_TO_STRING(COALESCE(g.bowl_preferences,'{}'),' '),ARRAY_TO_STRING(COALESCE(g.bar_preferences,'{}'),' '),g.allergies,g.notes)) LIKE '%'||LOWER($2)||'%') GROUP BY g.id ORDER BY COALESCE(MAX(o.closed_at),g.created_at) DESC`, [venueDbId, query, guestPeriodFrom]); return json(res, 200, { items: rows.map((row) => ({ ...row, phoneNumbers: row.phoneNumbers || (row.phone ? [{ label: 'Основной', number: row.phone, primary: true }] : []), tobaccoPreferences: row.tobaccoPreferences || [], bowlPreferences: row.bowlPreferences || [], barPreferences: row.barPreferences || [], loyaltyPoints: Number(row.loyaltyPoints || 0), visits: Number(row.visits || 0), totalSpent: Number(row.totalSpent || 0) })), periodDays: guestDays, periodFrom: guestPeriodFrom }); } catch (_) {} }
  ...
  if (clientLoyalty && req.method === 'POST') {
    ...
    if (repositories?.pool && /^[0-9a-f-]{36}$/i.test(client.id)) { try { await repositories.pool.query('UPDATE guests SET loyalty_points=$1 WHERE id=$2 AND venue_id=$3', [client.loyaltyPoints, client.id, venueDbId]); } catch (error) { client.loyaltyPoints = before; return json(res, 503, { error: 'loyalty_save_failed', detail: error.message }); } }
...
if (staffProfile && req.method === 'PATCH') {
  ...
  if (recipeCostPath && req.method === 'GET') {
    if (repositories?.pool && /^[0-9a-f-]{36}$/i.test(recipeCostPath[1])) { try { const { rows } = await repositories.pool.query('SELECT id,name,ingredients FROM inventory_recipe_cards WHERE id=$1 AND venue_id=$2 AND active=true', [recipeCostPath[1], venueDbId]); const recipe = rows[0]; if (!recipe) return json(res, 404, { error: 'recipe_not_found' }); const stock = (await repositories.inventory.list(venueDbId)).items; const lines = (recipe.ingredients || []).map((item) => { const stockItem = item.ingredientId ? stock.find((entry) => entry.id === item.ingredientId) : stock.find((entry) => entry.name.toLowerCase() === String(item.name || '').toLowerCase()); const quantity = Number(String(item.quantity || '').replace(',', '.').match(/-?\d+(?:\.\d+)?/)?.[0] || 0); return { ...item, stockName: stockItem?.name || item.name, unit: stockItem?.unit || null, quantity, unitCost: Number(stockItem?.cost || 0), cost: Math.round(quantity * Number(stockItem?.cost || 0) * 100) / 100, linked: Boolean(stockItem) }; }); return json(res, 200, { recipeId: recipe.id, lines, totalCost: Math.round(lines.reduce((sum, line) => sum + line.cost, 0) * 100) / 100, missing: lines.filter((line) => !line.linked) }); } catch (error) { return json(res, 503, { error: 'recipe_cost_unavailable', detail: error.message }); } }
